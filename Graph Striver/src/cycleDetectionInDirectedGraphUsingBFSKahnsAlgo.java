...
public class cycleDetectionInDirectedGraphUsingBFSKahnsAlgo {
// author: Tarun Verma
...
public static void solve() {
	...
	int vis[] = new int [n+1];
	for(int i =1;i<=n;i++) {
		vis[i] = indegree[i].size();
	}
	topoSort(vis, n);
}
static void topoSort(int vis[], int n) {
	...
	int cnt = 0;
	ArrayList<Integer> topologicalSortedOrder = new ArrayList<Integer>();
	while(!q.isEmpty()) {
		int top = q.poll();
		cnt++;
		for(int i: adj[top]) {
...
//	System.out.println(topologicalSortedOrder);
}
