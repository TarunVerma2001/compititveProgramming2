...
public static void topoSort(int node, int vis[], int n) {
	vis[node] = 1;
	for(pair i: adj[node]) {
		if(vis[i.fr] == 0) {
			topoSort(i.fr, vis, n);
...
public static void shortestPath(int src, int n) {
	int dis[] = new int [n];
	...
	while(!st.isEmpty()) {
		...
		if(dis[node] != (int)1e9+7) {
			for(pair i: adj[node]) {
				if(dis[i.fr] > dis[node] + i.sc) {
					dis[i.fr] = dis[node] + i.sc;
				}
			}
		}
