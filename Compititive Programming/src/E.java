...
// author: Tarun Verma
	 ...
	 */
	static long rec(int i, int j, int n, int m, int dp[][], int arr[][], long osf) {
		//base case 
		if(i == n || j == m) {
			return 0l;
		}
		...
		long ans = 0;
		if(i+1 < n) {
			ans += 1 + rec(i+1,j, n, m, dp, arr, osf);
		}
		if(j + 1 < m) {
			ans += 1 + rec(i, j+1, n, m, dp, arr, osf);
		}
		if((i+1 < n) && (j + 1 < m)) {
			ans += 1 + rec(i+1, j+1, n, m, dp, arr, osf);
		}
		return ans;
	...
	public static void solve() {
		...
		int arr[][] = new int[n][m];
		int dp[][] = new int[n][m];
		System.out.println(rec(0, 0, n, m, dp, arr, 0));
	}
